...
final_arima <- arima(car_sales.ts, order = c(2 , 1, 5 ), seasonal = list(order= c(1 , 1, 1), period=12))
#summary(final_arima)
#sarima.for(car_sales.ts,120,2,1,5,1,1,1,12)
forecast_10 <- forecast(final_arima, h=120, level=0)
plot(forecast_10)
```
```{r}
autoplot(car_sales.ts) +
  autolayer(forecast_10, color = "blue", series = "ARIMA") +
  theme_classic()

```
```{r}
accuracy(final_arima)
```


```{r}
#combine train and validation
my_final_model <- arima(car_sales.ts, order = c(2 , 1, 6 ), seasonal = c(1 , 1, 1))
my_final_model
summary(my_final_model)
#forecast next 10 years
forecast_10 <- forecast(my_final_model, h=120, level=0)
forecast_10
#plot
